...
with app.app_context():
 db.create_all()
 if not InvestmentStrategy.query.first():db.session.add(InvestmentStrategy(strategy_name="Safe & Steady",strategy_description="Low risk dummy strategy",portfolio_allocation="60% GLB, 40% VDE"));db.session.add(InvestmentStrategy(strategy_name="Aggressive Growth",strategy_description="High risk dummy strategy",portfolio_allocation="80% SSP, 20% WAY"));db.session.add(InvestmentStrategy(strategy_name="Tech Monopoly",strategy_description="Invest in future tech",portfolio_allocation="50% TYR, 50% SSP"));db.session.add(InvestmentStrategy(strategy_name="Consumer Staples",strategy_description="Everyday goods",portfolio_allocation="70% OCP, 30% SOY"));db.session.add(InvestmentStrategy(strategy_name="Mixed Bag",strategy_description="Diversified approach",portfolio_allocation="25% ACM, 25% INI, 25% DUM, 25% WAY"));db.session.commit()
 if not UserWish.query.first():db.session.add(UserWish(company_ticker="ACM",target_entry_price=120.50,additional_notes="Buy when it dips"));db.session.add(UserWish(company_ticker="INI",target_entry_price=60.00,additional_notes="Sell if it hits 60"));db.session.add(UserWish(company_ticker="TYR",target_entry_price=1200.00,additional_notes="Wait for a major crash"));db.session.add(UserWish(company_ticker="DUM",target_entry_price=35.00,additional_notes="Buy out competition"));db.session.commit()
MOCK_STOCK_MARKET_DATA=[{"ticker":"ACM","name":"Acme Corp","price":150.25,"change":"+1.5%"},{"ticker":"INI","name":"Initech","price":45.10,"change":"-2.3%"},{"ticker":"GLB","name":"Globex","price":305.50,"change":"+0.4%"},{"ticker":"VDE","name":"Vandelay Ind.","price":12.75,"change":"+5.1%"},{"ticker":"SSP","name":"Stark Ind.","price":890.00,"change":"-1.2%"},{"ticker":"WAY","name":"Wayne Ent.","price":450.20,"change":"+2.8%"},{"ticker":"OCP","name":"Omni Consumer","price":85.00,"change":"-0.5%"},{"ticker":"SOY","name":"Soylent Corp","price":12.50,"change":"+1.2%"},{"ticker":"TYR","name":"Tyrell Corp","price":1500.00,"change":"+10.5%"},{"ticker":"DUM","name":"Dunder Mifflin","price":42.00,"change":"+0.1%"}]
@app.route('/')
